...
process CREATE_LOUPE_INPUT {
    ...
    input:
        val run_id
        tuple val(id1), val(id2), val(anno1), val(anno2), path(h5ad1), path(h5ad2), path(samap_output)

    output:
        tuple val(id1), val(id2), path("${id1}_${id2}_LoupeInput.h5ad"), emit: loupeinput
        path "${run_id}_create_loupe_input.log", emit: logfile

    script:
    """
    LOG="${run_id}_create_loupe_input.log"
